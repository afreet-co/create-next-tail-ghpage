...
(async () => {
  const repositoryUrl = "https://github.com/afreet-co/next-tail-ghpage-ts.git";
  //Get the name of the app-directory to make
  const directoryName = process.argv[2];
  if (!directoryName || directoryName.match(/[<>:"\/\\|?*\x00-\x1F]/)) {
    return console.log(`
        Invalid directory name.
        Usage : 'create-next-tail-ghpage name_of_app'
        `);
  }
  ...
  let deployRaw = fs.readFileSync(deployPath, { encoding: "utf-8" });

  deployRaw = deployRaw.replace("/next-tail-ghpage-ts", `/${directoryName}`);
  removeFile(deployPath);
  fs.writeFileSync(deployPath, deployRaw);
  ...
  renameFile(
    `${directoryName}/.env.local.example`,
